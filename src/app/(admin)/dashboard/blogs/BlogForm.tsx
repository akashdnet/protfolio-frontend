...
export default function BlogForm({ initialData, onSubmit }: ProjectFormProps) {
  ...
  const [{ files, errors, isDragging }, { getInputProps, openFileDialog, handleDrop, removeFile }] =
    useFileUpload({ accept: "image/*", multiple: false, maxSize: 2 * 1024 * 1024 });
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        {/* rich text editor with primereact quile */}
        <FormField
          control={form.control}
          name="description"
          render={({ field }) => (
